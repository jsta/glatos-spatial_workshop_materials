...
We can see the contents of the `MGMTUNIT` field of our lines shapefile:
...
For example, we might be interested only in features that are of `MGMTUNIT` "footpath". Once we subset out this data, we can use it as input to other code so that that code only operates on the footpath lines.
...
`MGMTUNIT == footpath`. We can plot only the footpath lines:

```{r plot-subset-shapefile}
ggplot() + 
  geom_sf(data = zone_1) +
  ggtitle("Walleye Management Units", subtitle = "Zone 1") + 
  coord_sf()
```
...
ggplot() + 
  geom_sf(data = zone_1, aes(color = factor(OBJECTID)), size = 1.5) +
...
> > ```{r}
> > boardwalk_HARV <- erie_zones %>% 
> >   filter(MGMTUNIT == "boardwalk")
> > ```
...
```{r}
road_colors <- c("blue", "green", "navy", "purple", "orange")
```
...
ggplot() +
  ...
  labs(color = 'Unit ID') +
  ggtitle("Walleye Management Units", subtitle = "Roads & Trails") + 
  coord_sf()
```
...
We already know that we have four different `MGMTUNIT` levels in the erie_zones object, so we will set four different line widths.
...
Note that we could also use `aes(size = MGMTUNIT)` to tie the line thickness to the MGMTUNIT variable, so long as we had been careful to set factor levels appropriately. ggplot prints a warning when you do this, because it is not considered a good practice to plot non-spatial data this way.
