...
We can see that the elevation at our site ranges from `r minValue(erie_bathy)`m to
`r maxValue(erie_bathy)`m.
...
RGB_2m_df  <- as.data.frame(erie_bathy, xy = TRUE)
# make colnames easy to ref

ggplot() +
 geom_raster(data = RGB_2m_df , aes(x = x, y = y, fill = erie_bathy),
             show.legend = FALSE) +
...
RGB_2m_nas <- RGB_2m_df

ggplot() +
  geom_raster(data = RGB_2m_nas, aes(x = x, y = y, fill = erie_bathy)) + 
  scale_fill_gradient(low = 'grey90', high = 'blue', na.value = 'deeppink') +
  ggtitle("Lake Erie", subtitle = "Bathymetry - NA highlighted")

# memory saving
rm(RGB_2m_df, RGB_2m_nas)
```
...
DSM_highvals <- reclassify(erie_bathy, rcl = c(0, Inf, NA_integer_, -Inf, 0, 1L),
                           include.lowest = TRUE)
DSM_highvals <- as.data.frame(DSM_highvals, xy = TRUE)
DSM_highvals <- DSM_highvals[!is.na(DSM_highvals$erie_bathy), ]
...
ggplot() +
  ...
  annotate(geom = 'raster', x = DSM_highvals$x, y = DSM_highvals$y, 
           fill = scales::colour_ramp('deeppink')(DSM_highvals$erie_bathy)) +
  ggtitle("Lake Erie", subtitle = "Bathymetry - with values < 0m highlighted")
