...
Starting with this episode, we will be moving from working with vector
...
raster and how R handles these elements.
...
* **ellps=GRS80:** the ellipsoid (how the earth's  roundness is calculated) for
the data is GRS80
...
We can see that the elevation at our site ranges from `r minValue(erie_lld_agg)`m to
`r maxValue(erie_lld_agg)`m.
...
ggplot() +
 geom_raster(data = RGB_2m_df , aes(x = x, y = y, fill = erie_lld_agg),
             show.legend = FALSE) +
  scale_fill_gradient(low = 'black', high = "red") +
  ggtitle("Lake Erie", subtitle = 'Bathymetry') +
  coord_quickmap()
```
...
ggplot() +
  geom_raster(data = RGB_2m_df_nd, aes(x = x, y = y, fill = erie_lld_agg)) +
  scale_fill_identity() +
  ggtitle("Lake Erie", subtitle = "Missing data") +
  coord_quickmap()
...
RGB_2m_nas <- RGB_2m_df_nd

ggplot() +
  geom_raster(data = RGB_2m_nas, aes(x = x, y = y, fill = erie_lld_agg)) + 
...
DSM_highvals <- reclassify(erie_lld_agg, rcl = c(0, 400, NA_integer_, 400, 420, 1L),
                           include.lowest = TRUE)
DSM_highvals <- as.data.frame(DSM_highvals, xy = TRUE)
DSM_highvals <- DSM_highvals[!is.na(DSM_highvals$erie_lld_agg), ]

ggplot() +
  geom_raster(data = erie_lld_agg_df, aes(x = x, y = y, fill = erie_lld_agg)) + 
  scale_fill_viridis_c() + 
  # use reclassified raster data as an annotation
  annotate(geom = 'raster', x = DSM_highvals$x, y = DSM_highvals$y, fill = scales::colour_ramp('deeppink')(DSM_highvals$erie_lld_agg)) +
